...
static void _bt_drop_lock_and_maybe_pin(IndexScanDesc scan, BTScanPos sp) {
  ...
  if (IsMVCCSnapshot(scan->xs_snapshot) &&
      RelationNeedsWAL(scan->indexRelation) && !scan->xs_want_itup) {
 ...
 */
BTStack _bt_search(Relation rel, Relation heaprel, BTScanInsert key,
 ...
 */
Buffer _bt_moveright(Relation rel, Relation heaprel, BTScanInsert key,
                     Buffer buf, bool forupdate, BTStack stack, int access) {
  Page page;
  BTPageOpaque opaque;
  int32 cmpval;
      ...
      if (insertstate->postingoff != 0)
        ereport(
            ...
            (errcode(ERRCODE_INDEX_CORRUPTED),
             errmsg_internal(
                 "table tid from new index tuple (%u,%u) cannot find insert "
                 "offset between offsets %u and %u of block %u in index \"%s\"",
                 ItemPointerGetBlockNumber(key->scantid),
                 ItemPointerGetOffsetNumber(key->scantid), low, stricthigh,
 ...
 *----------
 */
int32 _bt_compare(Relation rel, BTScanInsert key, Page page,
                  OffsetNumber offnum) {
 ...
 *so->currPos. scan->xs_heaptid is set to the heap TID of the current tuple, and
 *if requested, scan->xs_itup points to a copy of the index tuple.
 *
  ...
  for (i = 0; i < keysz; i++) {
    ...
    if (cur->sk_flags & SK_ROW_HEADER) {
      ...
      if (cur->sk_subtype == rel->rd_opcintype[i] ||
        ...
        cmp_proc = get_opfamily_proc(rel->rd_opfamily[i], rel->rd_opcintype[i],
                                     cur->sk_subtype, BTORDER_PROC);
        if (!RegProcedureIsValid(cmp_proc))
          elog(ERROR,
               "missing support function %d(%u,%u) for attribute %d of index "
               "\"%s\"",
               BTORDER_PROC, rel->rd_opcintype[i], cur->sk_subtype,
               cur->sk_attno, RelationGetRelationName(rel));
        ScanKeyEntryInitialize(inskey.scankeys + i, cur->sk_flags,
                               cur->sk_attno, InvalidStrategy, cur->sk_subtype,
 ...
 *		previously returned.
 *
 *		On successful exit, scan->xs_heaptid is set to the TID of the
 *next heap tuple, and if requested, scan->xs_itup points to a copy of the index
 *tuple.  so->currPos is updated as needed.
 *
...
static bool _bt_readpage(IndexScanDesc scan, ScanDirection dir,
  ...
  if (ScanDirectionIsForward(dir)) {
    ...
    while (offnum <= maxoff) {
      ...
      passes_quals = _bt_checkkeys(scan, &pstate, arrayKeys, itup, indnatts);
      ...
      if (passes_quals) {
        ...
        if (!BTreeTupleIsPosting(itup)) {
           ...
           * TID
           */
          tupleOffset = _bt_setuppostingitems(
              so, itemIndex, offnum, BTreeTupleGetPostingN(itup, 0), itup);
          itemIndex++;
 ...
 * from currPos
 */
static inline void _bt_initialize_more_data(BTScanOpaque so,
                                            ScanDirection dir) {
  if (so->needPrimScan) {
